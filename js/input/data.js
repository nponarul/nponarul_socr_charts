socr.dataTable = function () {
    ...
    */
    var spreadSheet = {
      ...
      */
        ...
        firstColTitles: function () {
            var firstcol = $dataTable.handsontable('getDataAtCol', 0);
            spreadSheet.addRowHeaders(firstcol);
            $dataTable.handsontable('alter', 'remove_col', 0); 
            view.displayResponse(' Title successfully adjusted ', 'success'); 
        },
  ...
  */
    ...
    $controls.find('.firstcoltitles').on('click', spreadSheet.firstColTitles);
    $controls.find('.removecol').on('click', view.removeCol);
    $controls.find('#spreadsheet_sort').on('click', spreadSheet.sort);
    $controls.find('#select').on('click', spreadSheet.parseSelected);
    //$parse.find('#submit-parse').on('click', spreadSheet.loadData(uploaddata));
